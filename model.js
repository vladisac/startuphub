...
Parties.allow({
  ...
  update: function (userId, parties, fields, modifier) {
    return _.all(parties, function (party) {
      ...
      var allowed = ["title", "description", "x", "y", "date"];
...
"\n\nCome check it out: " + Meteor.absoluteUrl() + "\n"
  ...
  item: function (partyId, item) {
    if (! this.userId)
      throw new Meteor.Error(403, "You must be logged in to add items to an event");
    if (! _.isString(item))
      throw new Meteor.Error(400, "Invalid item");
    var party = Parties.findOne(partyId);
    if (! party)
      throw new Meteor.Error(404, "No such party");
    // if (party.owner !== this.userId)
    //   // only owner can add items
    //   throw new Meteor.Error(403, "Not your event to add items");

    var itemIndex = _.indexOf(_.pluck(party.items, 'user'), this.userId);
    if (itemIndex !== -1) {
      // update existing item entry

      if (Meteor.isServer) {
        // update the appropriate itme entry with $
        Parties.update(
          {_id: partyId, "items.user": this.userId},
          {$set: {"items.$.item": item, user: this.userId}});
      } else {
        // minimongo doesn't yet support $ in modifier. as a temporary
        // workaround, make a modifier that uses an index. this is
        // safe on the client since there's only one thread.
        var modifier = {$set: {}};
        modifier.$set["items." + itemIndex + ".item"] = item;
        Parties.update(partyId, modifier);
      }
      ...
      // add new item entry
      Parties.update(partyId,
                     {$push: {items:  {item:item, user:this.userId}}});
    }
  }
});
